...
// Retrieve all active inventory items with calculated stock
...
// Sync inventory.stock_quantity with calculated stock from stock_movements
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $calculated_stock = get_product_stock_from_batches($conn, $row['id']);
        if ((int)$row['stock_quantity'] !== $calculated_stock) {
            $sync_stmt = $conn->prepare("UPDATE inventory SET stock_quantity = ? WHERE id = ?");
            $sync_stmt->bind_param("ii", $calculated_stock, $row['id']);
            $sync_stmt->execute();
            $sync_stmt->close();
        }
    }
...
// Get near-expiration and expired products for alerts
    ...
    <div class="popup-content" style="max-width: 700px; max-height: 600px; overflow-y: auto;">
        <div class="form-container">
            ...
            <div style="margin-bottom: 15px;">
                ...
                <p id="exp_detail_product_name" style="margin: 5px 0;"></p>
            </div>
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 15px;">
                ...
                <tbody id="exp_detail_batches_table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <button type="button" class="action-btn" onclick="openExpirationDetails(<?php echo (int)$row['id']; ?>, '<?php echo htmlspecialchars($row['product_name'], ENT_QUOTES); ?>')">Expiration</button>
                </td>
            </tr>
            <?php endwhile; ?>
        </tbody>
...
function openExpirationDetails(productId, productName) {
    ...
    // Fetch batch data from server via AJAX
    fetch('get_expiration_batches.php?product_id=' + productId)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                populateExpirationDetails(data.batches);
            } else {
                alert('Error loading expiration details: ' + data.message);
            }
        })
        .catch(error => {
            console.error('Error:', error);
            alert('Failed to load expiration details');
        });
...
function populateExpirationDetails(batches) {
    ...
    batches.forEach(batch => {
        ...
        const dateAdded = createdAt ? createdAt.toLocaleDateString() + ' ' + createdAt.toLocaleTimeString() : 'N/A';

        const row = document.createElement('tr');
        row.innerHTML = `
            <td style="padding: 8px; border: 1px solid #ddd;">${batch.batch_reference || 'N/A'}</td>
            <td style="padding: 8px; border: 1px solid #ddd;">${batch.expiration_date || 'No expiration'}</td>
    ...
    document.getElementById('exp_detail_summary').textContent = `Total Active Qty: ${totalQty}`;
}
</script>
</body>
</html>
